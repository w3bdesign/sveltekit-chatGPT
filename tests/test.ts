...
test('GPT Chat and text submission', async ({ page }) => {
	...
	await page.waitForSelector('text=Hello! How can I help you today?');
  
	// Check if there is at least one output message
	const outputMessage = await page.getByText('Hello! How can I help you today?');
  
	expect(outputMessage).toBeVisible();
  });
